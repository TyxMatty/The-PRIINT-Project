...
let currentLang: 'pt' | 'en' = (localStorage.getItem("lang") as 'pt' | 'en') || 'pt';
...
function setLanguage(lang: 'pt' | 'en') {
    ...
    document.querySelectorAll('.label-min').forEach(el => (el as HTMLElement).innerText = translations.scaleMin[lang]);
    document.querySelectorAll('.label-max').forEach(el => (el as HTMLElement).innerText = translations.scaleMax[lang]);
}
...
function enableTTS() {
    if ('speechSynthesis' in window) {
        ...
        const btn = document.getElementById('btn-tts') as HTMLButtonElement;
        
        if (ttsEnabled) {
            if(btn) { btn.style.backgroundColor = 'var(--text-color)'; btn.style.color = '#000'; }
            ...
            window.speechSynthesis.cancel(); 
            if(btn) { btn.style.backgroundColor = 'var(--button-bg)'; btn.style.color = 'var(--button-text)'; }
        }
    } else {
...
function handleHomeNavigation(event: Event, targetId: string) {
    const questionnaireSection = document.getElementById('step-questionnaire');
                ...
                if (questionId) {
                    ...
                    setTimeout(() => {
                        ...
                        if (index + 1 < questionCards.length) {
                            ...
                            const nextText = (questionCards[index + 1].querySelector('legend') as HTMLElement)?.innerText;
...
(window as any).handleHomeNavigation = handleHomeNavigation;
