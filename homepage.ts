...
function goToQuestionnaire() {
    ...
    if (homepageContent && questionnaireSection) {
        ...
        setTimeout(() => {
            if (typeof (window as any).readCurrentScreen === 'function') {
                (window as any).readCurrentScreen();
            }
        }, 300);
    }
}
...
function generateScaleButtons() {
    ...
    scaleContainers.forEach((container, index) => {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            if (i === 1) btn.setAttribute('aria-label', '1 - Discordo Totalmente');
            if (i === 10) btn.setAttribute('aria-label', '10 - Concordo Totalmente');
            ...
            btn.addEventListener('click', () => {
                if (questionId) {
                    ...
                    setTimeout(() => {
                        if (questionCards[index]) questionCards[index].classList.remove('active'); 
    
                        if (index + 1 < questionCards.length) {
                            questionCards[index + 1].classList.add('active');
                            setTimeout(() => { (window as any).readCurrentScreen(); }, 100);
                        } else {
...
async function submitToAI(event: Event) {
    ...
    const submitBtn = document.querySelector('#final-submit-container .primary-btn') as HTMLButtonElement;
    if (!submitBtn) return;
...
function finalizeRegistration(event: Event) {
    event.preventDefault();
...
function changeLanguage(lang: 'pt' | 'en') {
    ...
    const setTxt = (selector: string, text: string) => {
    ...
    if (steps.length >= 3) {
        (steps[0].querySelector('.marca-texto') as HTMLElement).innerText = translations[lang].step1Title;
        (steps[0].querySelector('p') as HTMLElement).innerText = translations[lang].step1Text;
        (steps[1].querySelector('.marca-texto') as HTMLElement).innerText = translations[lang].step2Title;
        (steps[1].querySelector('p') as HTMLElement).innerText = translations[lang].step2Text;
        (steps[2].querySelector('.marca-texto') as HTMLElement).innerText = translations[lang].step3Title;
        (steps[2].querySelector('p') as HTMLElement).innerText = translations[lang].step3Text;
    }
    ...
    if (a11yBtns.length >= 4) {
        (a11yBtns[0] as HTMLElement).innerText = translations[lang].a11yBtnDefault;
        (a11yBtns[1] as HTMLElement).innerText = translations[lang].a11yBtnContrast;
        (a11yBtns[2] as HTMLElement).innerText = translations[lang].a11yBtnSoft;
        (a11yBtns[3] as HTMLElement).innerText = translations[lang].a11yBtnTTS;
    }
    ...
    if (authInputs.length >= 4) {
        (authInputs[0] as HTMLInputElement).placeholder = translations[lang].authName;
        (authInputs[1] as HTMLInputElement).placeholder = translations[lang].authEmail;
        (authInputs[2] as HTMLInputElement).placeholder = translations[lang].authPass;
        (authInputs[3] as HTMLInputElement).placeholder = translations[lang].authGender;
    }
    ...
    if (legends.length >= 10) {
        (legends[0] as HTMLElement).innerText = translations[lang].q1;
        (legends[1] as HTMLElement).innerText = translations[lang].q2;
        (legends[2] as HTMLElement).innerText = translations[lang].q3;
        (legends[3] as HTMLElement).innerText = translations[lang].q4;
        (legends[4] as HTMLElement).innerText = translations[lang].q5;
        (legends[5] as HTMLElement).innerText = translations[lang].q6;
        (legends[6] as HTMLElement).innerText = translations[lang].q7;
        (legends[7] as HTMLElement).innerText = translations[lang].q8;
        (legends[8] as HTMLElement).innerText = translations[lang].q9;
        (legends[9] as HTMLElement).innerText = translations[lang].q10;
    }
...
function enableTTS() {
    if ('speechSynthesis' in window) {
        ...
        const btn = document.querySelector('button[onclick="enableTTS()"]') as HTMLButtonElement;
...
function readCurrentScreen() {
    ...
    if (a11ySection && a11ySection.style.display !== 'none') {
        ...
        const desc = (document.querySelector('#step-accessibility p') as HTMLElement)?.innerText || "";
        textToRead = title + ". " + desc;
    } 
    else if (questSection && questSection.style.display !== 'none') {
        const activeLegend = document.querySelector('.question-card.active legend') as HTMLElement;
        if (activeLegend) {
...
(window as any).enableTTS = enableTTS;
(window as any).readCurrentScreen = readCurrentScreen;
(window as any).goToQuestionnaire = goToQuestionnaire;
(window as any).submitToAI = submitToAI;
(window as any).finalizeRegistration = finalizeRegistration;
(window as any).openA11yModal = openA11yModal;
(window as any).closeA11yModal = closeA11yModal;
(window as any).setTheme = setTheme;
(window as any).initScrollAnimations = initScrollAnimations;
